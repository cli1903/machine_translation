...
def Self_Attention(K, V, Q, use_mask=False):
    ...
    dk = K.shape[2]
    scaled = att_weights / (dk ** 0.5)
    if use_mask:
        scaled += atten_mask
    weighted = tf.matmul(tf.nn.softmax(scaled), V)
...
class Atten_Head(tf.keras.layers.Layer):
    def __init__(self, input_size, output_size, use_mask):        
        ...
        self.W_k = self.add_weight(shape = [input_size, output_size], trainable = True, dtype = tf.float32)
        self.W_v = self.add_weight(shape = [input_size, output_size], trainable = True, dtype = tf.float32)
        self.W_q = self.add_weight(shape = [input_size, output_size], trainable = True, dtype = tf.float32)
    ...
    def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
        ...
        return Self_Attention(K, V, Q, use_mask = self.use_mask)
