...
def get_data(french_training_file, english_training_file, french_test_file, english_test_file):
    ...
    ftrain, etrain = pad_corpus(ftrain, etrain)

    #3) Pad testing data (see pad_corpus)
    ftest, etest = pad_corpus(ftest, etest)

    #4) Build vocab for french (see build_vocab)
    french_vocab, fpad_id = build_vocab(ftrain)

    #5) Build vocab for english (see build_vocab)
    english_vocab, eng_pad_id = build_vocab(etrain)

    #6) Convert training and testing english sentences to list of IDS (see convert_to_id)
    train_english = convert_to_id(english_vocab, etrain)
    test_english = convert_to_id(english_vocab, etest)

    #7) Convert training and testing french sentences to list of IDS (see convert_to_id)
    train_french = convert_to_id(french_vocab, ftrain)
    test_french = convert_to_id(french_vocab, ftest)

    return train_english, test_english, train_french, test_french, english_vocab, french_vocab, eng_pad_id
